...
async def reminder_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, loan in ACTIVE_LOANS.items():
        ...
        if now >= reminder_time and not loan.get('reminded'):
            try:
                await context.bot.send_message(chat_id=uid, text=f"⏰ **تذكير:** يتبقى يوم واحد على موعد إرجاع كتاب ({loan['book']}). يرجى تسليمه للمكتبة.")
                loan['reminded'] = True
                save_data()
            except: pass

# --- 4. منطق البوت (الترحيب، البيانات، الاستعارة) ---
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == 'NAME':
        ...
        if re.match(r'^0\d{9}$', text):
            ...
            await update.message.reply_text("✅ تم التسجيل بنجاح. اختر قسماً لتصفح الكتب:", reply_markup=InlineKeyboardMarkup(kb))
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cat_"):
        ...
        kb = [[InlineKeyboardButton("✅ طلب استعارة", callback_data=f"terms_{book}")]] if book not in BORROWED_BOOKS and user_id not in ACTIVE_LOANS else [[InlineKeyboardButton("🔙 رجوع", callback_data="back_to_cats")]]
        ...
        if uid in ACTIVE_LOANS:
            ...
            save_data()
            await query.edit_message_text("✅ تم تأكيد استلام الكتاب.")
...
def main():
    ...
    # تنظيف التحديثات المعلقة لمنع Conflict
