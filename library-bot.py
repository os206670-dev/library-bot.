...
# --- 1. إعداد الويب سيرفر (Flask) لحل مشكلة Render ---
...
def run_flask():
    # Render يطلب فتح منفذ (Port) وإلا سيعتبر التطبيق متوقفاً
...
TOKEN = '8461701738:AAE4g9ukO2IkiPX2PEJJzOdw29JP_sPEWDs' 
...
LIBRARY_DATA = {
    ...
    'سلسلة آفاق العلمية': {
        ...
        'الزلازل والبراكين': 'أسباب اهتزاز الأرض وانفجار الحمم من باطنها.',
        'الحرائق والفيضانات': 'أخطار الطبيعة وكيفية مواجهتها والحد من أضرارها.'
    },
    'العلوم الشرعية والتاريخ': {
        'السيرة النبوية': 'قصة حياة الرسول ﷺ بأسلوب تاريخي ممتع.',
        'الصحيح': 'مرجع للأحاديث النبوية الصحيحة.',
        'تفسير العشر الأخير': 'شرح ميسر لمعاني سور القرآن الكريم في الأجزاء الأخيرة.',
        'الفرح بالقرآن': 'الراحة النفسية والإيمانية في تدبر القرآن.',
    ...
    'روايات وكتب أدبية': {
        ...
        'عندما تكون الغربة أوطاناً': 'خواطر حول البحث عن الانتماء والسلام الداخلي.',
        'قفص الموت الغامض': 'رواية إثارة وغموض تتطلب ذكاءً لحل لغزها.',
        'قواعد البحث العلمي': 'دليل تعليمي لكتابة البحوث المدرسية المنظمة.'
    }
}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✨ مرحباً بك في نظام مكتبة ابن العميد الذكي\n\nيرجى إرسال اسمك الثلاثي للبدء:")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == 'NAME':
        ...
        if re.match(r'^0\d{9}$', text):
            ...
            await update.message.reply_text("✅ تم التسجيل بنجاح.\nاختر قسماً لتصفح الكتب:", reply_markup=InlineKeyboardMarkup(kb))
        else:
            await update.message.reply_text("❌ رقم غير صحيح، يرجى كتابة 10 أرقام تبدأ بـ 0:")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cat_"):
        ...
        kb = [[InlineKeyboardButton("✅ استعارة", callback_data=f"terms_{book}")]] if book not in BORROWED_BOOKS and user_id not in ACTIVE_LOANS else [[InlineKeyboardButton("🔙 رجوع", callback_data=f"cat_{cat}")]]
        await query.edit_message_text(f"📖 {book}\n\n{desc}", reply_markup=InlineKeyboardMarkup(kb))

    elif data.startswith("brw_"):
        book = data.split("_", 1)[1]
        now = datetime.now()
        ret = now + timedelta(days=7) # مدة الاستعارة أسبوع
